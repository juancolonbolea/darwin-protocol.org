...
date: 2026-08-28
owns: |
  Formal logical structure of Darwinex. 7 primitives (P0-P6), 11 axioms,
  74 active theorems (63 positive + 11 failure), 1 lemma (L1), 21 proofs.
...
supersedes: "CONSTITUTION v13 (28 Aug 2026)"
cross_refs:
  - PROTOCOL
  - DEVELOP
  - REWARD
...
audit_trail: |
  v14 (28 Aug 2026): A17 (Attestation and custody are separable) adopted as an active structural axiom, restated to its irreducible separation claim and grounded P1+P6+A2+A9, force C. A2 gains an estimand-scope clause naming the events its estimator reads. T62 (Regulatory-Surface Theorem, with a jurisdiction hedge and a stated falsifier) and T63 (Settlement Theorem, corollaries T63.1 settlement-not-scoring layer and T63.2 the fee is a leg) added; T63's cycle atomicity is stated as a conformance requirement on the settlement and matching components (`analysis/components/`), consistent with the demotion of A16. F11 (Anchor Compromise, ¬A17) added. G23 (the anchor set is unenumerated) opened. §0 unchanged: the restated A17 states what certification requires rather than a commitment the venue makes, so the framework's normative content still sits in exactly three places (P6, A9, A14). 11 axioms, 74 theorems (63 positive + 11 failure), 1 lemma, 21 proofs, 2 empirical anchors.
...
# The Darwinex Constitution — v14

> 7 primitives. 11 axioms. 74 active theorems (63 positive + 11 failure). 1 lemma (L1). 21 proofs. 2 load-bearing empirical anchors (E1, E2). 4 named principles. Change history: frontmatter audit trail (this file, §Axiom Audit Trail and §Primitive Audit Trail).
...
| **A2** | **Merit is estimable** | A protocol-computable estimator of one agent's contribution exists, separating it from luck, beta and access at finite sample, and informative when the agent knows it is computed. The estimator's inputs are the events of the economic loop it certifies — allocation in, live fill, realised outcome, payout to the rightful owner — observed, not held. | P1, P6 | **e** | [[DEVELOP]], [[PHENOTYPE--risk-engine]] |
...
| **A17** | **Attestation and custody are separable** | Attestation and custody are separable: certifying an economic loop requires observational access to its events, anchored against a reference the protocol does not control, and no proprietary interest in the assets moved. Three conditions, each necessary: **observational access** (the certifier is present at the loop's events, not reconstructing them after the fact), **an external anchor** (what is observed is checked against a reference outside the certifier's control), and **no proprietary interest** (the certifier does not stand to gain or lose on the assets whose movement it attests). Custody appears in none of them, which is the claim: holding what one certifies is neither necessary for certification nor compatible with the third condition. This generalises A9's arm's-length principle from the venue to the whole money loop — A9 forbids taking the other side of member flow inside the venue; A17 extends arm's length to every point the assets pass through, since a certifier with a claim on the assets is a counterparty to the loop it grades. What A2 supplies is that merit *can* be estimated; A17 adds that the estimator need not, and must not, hold what it reads. Not derivable from A2, A9 or P0-P6: none contains a possession predicate. **Consequences (theorem-derivable):** T62 (the prudential surface attaches to custody, not to attestation), T63 (the certified payout loop closes over a custody chain the protocol does not own). ¬A17 is F11. | P1, P6, A2, A9 | **e→X membrane** | [[PROTOCOL]] |
...
| **A2** | **SCOPE CLAUSE ADDED v14 (28 Aug 2026)** | One sentence fixes the estimand: the estimator's inputs are the events of the economic loop it certifies — allocation in, live fill, realised outcome, payout to the rightful owner — observed, not held. The clause was carried inside the 4-Jun A17 formulation, where it made A17's path-presence half circular: A17 supplied the scope A2 needed, and only A17's own consequence theorem fixed that scope. Sited on A2, path presence follows from the estimator being protocol-computable and stops being axiomatic. A17 keeps only the separation claim; the two rows do not restate each other. |
| **A17** | **ADOPTED v14 (28 Aug 2026), restated** | Admitted as an active structural axiom. The 4-Jun formulation bundled five things: a scope clause on A2, an irreducible separation claim, a mechanism, a dated corporate posture, and its own consequences. The scope clause moves to A2; the mechanism (which references qualify as anchors, how the settlement path is ridden) moves to the component layer; the posture is dropped as not a protocol claim; the consequences are T62 and T63. What remains is the separation claim, and it is irreducible: A9 reaches intent and the other side of member flow but never possession, A13 governs information release rather than holding, P6 orders reward, and no primitive P0-P6 contains a possession predicate. The best bridge from A9 — custody is a capability, and self-binding should remove capability rather than intent — is prudential and proves too much, so it does not carry entailment. A17 is exempt from no part of the framework's machinery: it takes force C, ¬A17 is stated as F11, and its unenumerated anchor set is opened as G23. |
...
| **C** (Conservation) | Normative: preserves | A1 (negation), A9, A17, P6 | Preserve the fitness function. Merit, not extraction, determines outcomes; the venue binds itself against its own governors; and what the venue certifies is anchored outside its own control and outside any interest of its own in the assets. | Extraction — compounding serves the wrong people. Self-certification — the record grades itself (F11) |
...
- **A17 sits in C as a conservation constraint, not a normative commitment.** Membership in C is not a normativity test — A1's negation sits there too, and A1 is descriptive. C preserves the fitness function, and A17 preserves the one thing that makes the fitness function readable: an attestation anchored where the protocol cannot reach it. What A17 states is what certification *requires*, so the self-binding that follows is conditional on wanting to certify at all. §0's three normative places are therefore unchanged.
- **Axiom distribution reflects operational maturity:** V has 4 axioms (A1, A2, A3, A13) and S has 3 (A4, A8, A14) — the built side, which the cage funded. I and P have one axiom plus one theorem each (A5 + T56, A7 + T9) — the growth edge, which the cage blocked. C carries the two structural normative commitments (A9, P6, plus the negation of A1) and A17's custody boundary. The CIF transfer releases I and P.
- **One failure mode per operational force, two for Inheritance:** F1 = I fails at the α channel, F10 = I fails at the retention channel, F2 = P fails, F4 = S fails, F5 = V fails. F3 = all fail simultaneously (death spiral). F11 is the exception: it is C's failure at the attestation anchor rather than any operational force's.

---

## III. THEOREMS — Derived Claims (74 active: 63 positive + 11 failure) + 1 lemma
...
### Attestation & Settlement Boundary (28 Aug 2026 — v14)

| # | Theorem | Derived from | Status |
|---|---------|-------------|--------|
| **T62** | **Regulatory-Surface Theorem** — **in jurisdictions where the regulated perimeter attaches to custody and capital**, the prudential, cyber-attack and balance-sheet surface attaches to *holding* the assets, not to *attesting* to their movement. Producing verified information about an economic loop (A17) does not, of itself, place the producer inside that perimeter, assemble a pool of client assets worth attacking, or load the balance sheet with exposure that is not the producer's own; holding the assets does all three. ∴ a certifier that observes the loop without holding what moves through it carries a strictly smaller custodial-prudential surface than one that does both. This is what makes certification embeddable inside a party that does custody: that party carries the custody surface, the certifier carries the attestation. Regulatory scope still escalates with aggregation level (T33) — but along the custody axis, not the attestation axis. **Boundary condition:** attestation that crosses into discretion, possession or guarantee re-acquires the surface. The perimeter follows the assets, wherever they land. **Scope — what T62 does not claim.** Attestation attracts regulation of its own: rating agencies, benchmark administrators and auditors are supervised without custodying anything. T62 claims the two surfaces are *distinct*, and that the custodial one is shed by not holding — not that a certifier is unregulated. **Falsifier:** T62 fails where a certifier that holds no client assets, exercises no discretion over them and gives no guarantee is nonetheless brought inside the custodial-prudential perimeter — capital requirements, client-asset rules, resolution planning — by the act of attestation alone; and it fails as a general claim where that is the ordinary treatment rather than an outlier jurisdiction. The claim is empirical and jurisdiction-dependent, and is stated with its falsifier for that reason. | A17+T32+A9+P6 | ✅ |
| **T63** | **Settlement Theorem** — the certified payout-to-owner loop (allocation in → live fill → realised outcome → payout to the rightful owner: investor return plus provider fee) closes over a custody chain the protocol does not own — a full-stack broker, a third-party custodial rail, an exchange, an on-chain settlement contract — or over a settlement ledger the protocol reads directly. The protocol issues, clears and settles units of trust across that money rail while holding none of what moves along it (A17). **Cycle atomicity is a conformance requirement, not axiom content:** that each cycle's postings commit whole or not at all, with no partial commit externally visible, binds the settlement and matching components — see the component decomposition in `analysis/components/` (component 4, Settlement Layer, invariant I4; component 3, Matching Engine) — consistent with the demotion of A16, which asserted the same property one layer too high. **Corollary T63.1 (settlement layer, not scoring layer):** what is certified is a closed economic loop, not a track record. The protocol attests provenance × realisation × *settlement* — the loop closing to the owner — which makes it a settlement and matching layer rather than a custodian or a ratings agency. A scoring layer grades outcomes it did not see settle; a settlement layer sees them settle and is what T63.2 then makes payable. **Corollary T63.2 (the fee is a leg, not a deduction):** the performance fee is a leg of the certified payout loop, settled through the custody chain and attested in the settlement path, rather than deducted from assets the protocol holds. A certifier with no custody has no deduction point; a certifier present in the settlement path collects through the path it already attests. ∴ multi-venue and multi-asset operation, and certification embedded as a module inside a custodying partner, are structural consequences rather than aspirations. | A17+T42+T9 | ✅ |

### Failure Theorems (v7, 11 Apr 2026; v10 extension 19 Apr 2026)

The framework diagnoses its own breakdown via two failure classes. **Class A — internal force collapse (F1-F5, F10):** one failure mode per operational force, two for Inheritance. F1 = I fails at the α channel (α-Collapse), F10 = I fails at the retention channel (Surplus Not Retained), F2 = P fails (β-Collapse), F3 = all fail (Death Spiral), F4 = S fails (★-Collapse), F5 = V fails (Stagnation). On the three-circle visual: F1-F2 = bridges break (circles stop converging), F3 = e circle contracts (anti-concentricity), F4 = ★ center fails (venue degrades to intermediary), F5 = e circle ossifies (no new variation enters), F10 = the circles hold their size and stop growing (surplus rescued, never redeployed). **Class B — adversarial / external (F6-F9, v10):** F6 = dataset poisoning, F7 = reflexivity (Soros), F8 = regulatory weaponisation, F9 = strategy convergence / monoculture. Class A is self-inflicted; Class B is externally induced or structurally emergent despite honest operation. **F11 (Anchor Compromise) belongs to neither cleanly** — the reference an attestation rests on can be captured from inside (the protocol anchors against its own record) or from outside (the reference sits with a party holding an interest in the assets), so it is stated as a failure of the attestation anchor rather than of a force. **Each failure mode is an information pathology** — a stage of the information lifecycle (Generate→Create→Protect→Liberate) that breaks.
...
| **F11** | **Anchor Compromise (Self-Certification) — Information pathology: SELF-REFERENCE** — If the reference the attestation is anchored against is controlled by the protocol, or held by a party with a proprietary interest in the assets moved (¬A17), the attestation collapses to self-certification: the loop is graded against a record its grader can write. **Consequence for A2:** the estimator's inputs cease to be observed events of the economic loop and become entries under the estimator's own control, so the separation of contribution from luck, beta and access becomes unverifiable at any sample size — A2's finite-sample claim survives as arithmetic and loses its ground. **Consequence for A7's β:** outsiders allocate on proof they cannot check. β stops measuring the response of capital to evidence and starts measuring its response to the venue's reputation, so it holds while the reputation does and falls discontinuously when it does not — a fragility invisible in the coefficient itself. E1 (T42) is the residual defence and the only one: an external benchmark the protocol does not administer is still an anchor, which is why a compromised anchor is survivable exactly as far as T42 reaches and no further. **Distinct from F6:** under F6 the inputs are poisoned by adversarial providers while the reference stands, so a robustness threshold on provider behaviour helps; under F11 the reference itself is compromised, and no such threshold reaches it. **Information stage that breaks: VERIFICATION** — the attestation carries information about its author rather than about the loop. **Diagnosis:** anchor references resolving to entities the protocol controls or is a counterparty to; attested outcomes reconcilable only against the protocol's own records; no published gap between attested outcomes and independently reconstructable ones. **Intervention:** re-anchor to a reference outside the protocol's control and outside any party with an interest in the assets; publish the reconciliation between attested and independently observable events; treat the anchor set as a governed parameter (G23). | ¬A17+A2+A7 | ✅ |
...
AXIOMS (11 active; A6/A10/A11/A12/A15/A16 and old-A15 demoted — numbering gaps kept)
  ...
  STRUCTURAL:   A4←P0,P4(surplus; per-dimension capacity; separable)  A5←P3,A4(retention)
                ...
                A17←P1,P6,A2,A9(attestation and custody are separable: observational access + an anchor
                     outside protocol control + no proprietary interest in the assets moved;
                     A9's arm's length generalised from the venue to the whole loop; ¬A17→F11)
  ...
  FORCE MAPPING: V←A1,A2,A3,A13  S←A4,A8,A14  I←A5,T56  P←A7,T9  C←A1(¬),A9,A17,P6

THEOREMS (74 active: 63 positive + 11 failure, T4 absorbed)
  ...
  T62←A17,T32,A9,P6  (Regulatory surface attaches to custody and capital, not to attestation;
                      jurisdiction-hedged, falsifier stated)
  T63←A17,T42,T9  (Settlement theorem — the certified payout loop closes over a custody chain the
                   protocol does not own; cycle atomicity is a component conformance requirement,
                   not axiom content; T63.1 settlement layer not scoring layer; T63.2 fee is a leg)
  ...
  F11←¬A17,A2,A7  (anchor compromise — attestation collapses to self-certification; A2's estimator
                   loses its ground, A7's β measures reputation rather than evidence)
```
...
| G23 | **The anchor set is unenumerated** — A17 requires attestation to be anchored against "a reference the protocol does not control," but which references qualify is nowhere stated, so the axiom's central term has no membership test. Needs: the test itself (independence of control, absence of a proprietary interest in the assets moved, and observability of the access the reference grants); the treatment of a reference that is independent of the protocol yet held by a party with its own interest in the assets, which is the common case rather than the edge case; and a rule for what happens to standing attestations when an anchor is withdrawn, since observational access is granted by a counterparty and is revocable. Until the set is defined, A17 trades counterparty risk for reference-integrity risk without pricing the second. | 🔴 | **OPEN (v14).** F11 states what a compromised anchor costs; G23 is the missing definition of what makes one sound. Couples to G21: the access an anchor grants is the same disclosure surface A13's granularity and lag bound, so the two parameters must be set together. |
...
| **Axioms** | **11** | A1, A2, A3, A4, A5, A7, A8, A9, A13, A14, A17. **v14:** A17 (Attestation and custody are separable) adopted as an active structural axiom, restated to its separation claim, grounded P1+P6+A2+A9 and assigned force C; A2 gains the estimand-scope clause A17 used to carry. **v13:** the other ten restated to their irreducible claims; A10 demoted to vocabulary; A15 and A16 demoted to the component conformance spec; A6/A11/A12/old-A15 demoted earlier. Numbering gaps kept — see §Axiom Audit Trail. |
...
| **Open gaps** | **23 (G1–G23)** — G10, G20, G21, G23 open; G22 deferred; the rest resolved | **v14:** +G23 (the anchor set is unenumerated — A17's central term has no membership test; couples to G21). **v13:** +G20 (β estimator undefined — highest-value follow-up), +G21 (A13 granularity/lag unparameterised), +G22 (P3↔X redeployability, deferred per Q2). G17-G19 resolved by T50-T52 (v8). G14 resolved (T32 reframed as normative). G15 resolved (T48 formalizes general principle). G1-G3, G5-G9 resolved (12 Apr cascade). G4 resolved (two-layer economy). G11-G13, G16 resolved earlier. |
...
*Audit trail: v14 (28 Aug 2026) A17 (Attestation and custody are separable) adopted as an active structural axiom, restated to its separation claim and grounded P1+P6+A2+A9, force C; A2 gains an estimand-scope clause; T62 (Regulatory-Surface, with jurisdiction hedge and falsifier) + T63 (Settlement, corollaries T63.1/T63.2, atomicity as a component conformance requirement) + F11 (Anchor Compromise, ¬A17) added; G23 opened; §0 left unchanged, since the restated A17 states what certification requires rather than a commitment the venue makes. 11 axioms, 74 theorems (63 positive + 11 failure), 1 lemma, 21 proofs, 2 empirical anchors. v13 (28 Aug 2026) axiom stress test applied: 13 axioms → 10 (A10 → vocabulary; A15, A16 → component conformance spec; the remaining ten restated), P1/P2 strips, §0 admits A9 + A14 as structural normative commitments, A3 states the no-rival-venue assumption, L1 + F10 + E2 added. 10 axioms, 71 theorems (61 positive + 10 failure), 1 lemma, 21 proofs, 2 empirical anchors. v12 (20 Apr 2026) A15 Intent Conservation + A16 Atomic Cycle. T42 bilateral extension. T61 promoted. T64 Patience Lever. 3 Named Principles. 13 axioms, 70 theorems, 21 proofs. v11 (19 Apr 2026) T58 (Open-Trust↔INDX duality), T59 (continuous irreplaceability), T60 (Bellman attractor), T42 elevated as load-bearing empirical anchor, P6 value-rootedness made explicit, IMR "phase transition" softened to inflection. 68 theorems, 21 proofs. v10 A7 β-only, T56/T57, F6-F9. v9 Force-axiom mapping, F5. v8b Protocol synthesis. v7 audit hardening. v6 P0 regrounding. v5 certification chain. v4 protocol transition. v3 moat + network.*
